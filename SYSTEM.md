...
### 0.1 现实主义题材第一原则：真实性是生命线
**如果写的是现实主义题材（不是玄幻/科幻/架空），人物的收入、支出、工作流程、行为逻辑、社会运转规则，必须全部有真实调研依据，不许想当然编。**
- 硬数字（收入、垫资、金额、距离、时间）必须有调研来源，不许编
- 硬流程（一天怎么过、一件事怎么办、钱怎么流转）必须符合现实，不许想当然
- 硬逻辑（人物遇到事第一反应、决策逻辑、社交圈）必须符合身份，不许为了推进剧情让人物做不符合身份的事
- 缺调研先补调研——任何策划/写作环节，如果需要用到没调研过的内容，必须先做WebSearch补充调研，不许直接编
- 不许先想剧情链条再套人物身份——必须先把人物真实生存状态搞清楚，故事从真实状态里自然长出来
- 所有审稿环节（选题/开书/提纲/正文）必须先检查真实性/调研充分性，再检查AI味，再检查其他
- 调研不扎实的方案/策划/提纲，必须打回补调研，不能放水
...
4. 提纲最后附：本批情绪曲线、人物状态变化、时间推进、**九问自检结果（逐条打勾）**、补调研结果（如果有）
5. 保存到 work/<novel_slug>/batch_plans/batch001_outline.md
6. **检查自检结果**：检查提纲末尾有没有outline_writer的九问自检清单（逐条打勾，真实性检查必须全过），没有的直接打回补自检
7. **提纲独立审稿**：委派子Agent读 prompts/review/outline_reviewer.md，对提纲做全面审稿
   - 审稿重点顺序：①真实性/调研充分性检查（最优先级，硬数字/硬流程/硬逻辑）→ ②AI组合拳套路检查 → ③状态延续 → ④人物OOC → ⑤伏笔操作 → ⑥节奏 → ⑦行当真实性 → ⑧素材使用
   - 检查提纲末尾有没有补调研结果（如果涉及新领域/新地方/新交易，必须附补调研）
   - 审稿结果处理：
...
1. **最高原则升级**：把"真实性是现实主义题材的生命线"作为0.1最高原则，优先级高于风格规则和反AI
2. **三硬原则**：所有策划/写作必须遵守硬数字（有来源）、硬流程（符合现实）、硬逻辑（符合身份），不许想当然
3. **topic_planner大幅强化调研要求**：调研从3个来源提升到至少5个，必须搞清楚收入/支出/垫资/流程/决策逻辑，所有数字必须附来源
4. **opening_planner增加强制补充调研环节**：开书策划前必须先读调研报告，涉及跨领域内容（如包工头卖古董）必须先补WebSearch调研，再开始设计
5. **outline_writer真实性铁则**：缺调研先补调研，不许凭空加设定（上一章没提过的东西不能突然冒出来），八问升级为九问，真实性检查放第0位
6. **三个审稿Agent全部升级**：topic_reviewer/opening_plan_reviewer/outline_reviewer都把"真实性/调研充分性检查"作为🔴最优先级，比AI检查还重要
7. **"先想剧情再套人物"列为头号禁止项**：用"矿场倒闭→包工头卖古董→去天津→被骗→被诈骗"作为典型反面案例，明确禁止先想剧情链条再硬套人物身份
8. **人物行为逻辑校验**：人物遇到事的第一反应必须符合身份（包工头缺钱第一反应是要账/找关系/变卖生产资料，不是卖古董）
9. **跨领域内容必须额外调研**：主角接触本职以外的领域（古董、金融、法律等），必须单独调研该领域，不许想当然
10. **模糊描述禁止**：不许用"收入不错""欠了不少钱""挺值钱的"这种模糊话，必须给具体数字/范围+来源

**v4.4变更（重大系统漏洞修复）：**
1. 补全提纲独立审稿环节、审稿→修改→重审循环
2. 强化AI组合拳套路检查、强制自检输出、状态更新校验
3. 补全完结审稿、总纲扩展审稿、Prompt索引补全
4. 素材库路径明确、开书策划审稿通过后再初始化Bible等
