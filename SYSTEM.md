...
[2. 开书准备]（根据创作模式不同走不同路径）
   ...
   模式C：总编指定方向 → 选题策划（围绕方向出2-3个差异化方案）→ 总编选一个 → 定题补充调研
   → 审阅通过
   ↓
[3. 开书策划]
   → 人物卡+世界观+结构设计+前20章粗纲+初始化动态状态文件
   → 开书策划审稿（opening_plan_reviewer）→ 修改/重写 → 审阅通过
   → 初始化Novel Bible（复制风格规则、创建meta.yaml、Git提交）
   ↓
[4. 试写（首批2-3章）]
   → 写提纲（读取素材库materials/）→ 审阅通过
   → 写正文+内部审稿+状态更新 → 修改/重写 → 审阅通过
   ↓
[5. 循环写作（每批N章，N由总编定，默认3）]
   → [模式A] 搜集近7日热点 → 审阅
   → 写下一批提纲（读取素材库materials/中未使用的素材）→ 审阅通过
   → 写正文+内部审稿+状态更新 → 修改/重写 → 审阅通过
...
4. **开书策划审稿**：委派子Agent读 prompts/review/opening_plan_reviewer.md，对opening_plan.md做全面审稿
   - 审稿维度：风格一致性、人物、入口事件与传导网络、暗线与伏笔、前20章节奏、反AI设计感、世界/行当真实性、生长空间
   - 审稿结果：通过/修改后再审/不通过需重做
   - 如果审稿发现🔴严重问题，让opening_planner子Agent修改后重审
   - 如果只有🟡一般问题和🟢建议，整理后呈现给总编参考
5. 呈现给总编：
