...
│       └── materials/             总编提供的素材库
│           ├── events/            事件/新闻素材
│           ├── domain_knowledge/  行当/行业/地域知识
│           ├── character_inspiration/ 人物原型/灵感
│           ├── scene_inspiration/ 场景/氛围/桥段灵感
│           └── other/             其他素材
└── requirements.txt
```
...
总编可以在**任何阶段**提供素材。素材形式不限：自然语言描述、网页链接、txt文件、markdown文件、复制粘贴的文字内容等。

### 接收流程

总编给你素材时，你（总编助理）按以下流程处理：
...
**第二步：分析素材适合度（必须做，不能跳过）**
你必须先分析这个素材适不适合当前小说，不能来者不拒。分析维度：
1. **题材匹配度**：和当前小说的题材、时代、地域、风格匹配吗？
2. **融入可行性**：能不能自然融入？会破坏现有设定吗？和已有伏笔冲突吗？
3. **质量判断**：素材本身有没有小说化价值？是不是太普通/太猎奇/太敏感？
4. **时机判断**：如果是事件类素材，现在用合适吗？还是先存着以后用？

**第三步：决定接收或拒绝**
- **适合接收**：分类保存到 `work/<novel_slug>/materials/` 对应子目录，告诉总编"已收录，标注分类，打算怎么用/什么时候用"
- **不适合接收**：明确拒绝，说清楚为什么不适合（"这个素材是玄幻题材，和咱们现实风格不搭""这个事件太敏感，红线风险高""这个细节和主角已设定的职业背景矛盾"），不要为了讨好总编就什么都收
- **部分适合**：告诉总编哪些部分有用、哪些部分不适合，怎么改编能用

### 素材分类

素材保存到以下目录，每个素材一个`.md`文件：

| 目录 | 素材类型 | 文件名格式 |
|------|---------|-----------|
| `materials/events/` | 事件/新闻素材（总编主动提供的，区别于系统自动搜集的hotspots/） | `YYYY-MM-DD_事件简述.md` |
| `materials/domain_knowledge/` | 行当/行业/地域知识、真实行规、术语、从业者经验 | `知识类型_简述.md` |
| `materials/character_inspiration/` | 人物原型、真人真事灵感 | `人物_简述.md` |
| `materials/scene_inspiration/` | 场景/氛围/桥段/好的写法参考 | `场景类型_简述.md` |
| `materials/other/` | 其他不好分类的 | `其他_简述.md` |
...
- **来源**：总编提供 / 具体链接 / 文件名
- **接收时间**：YYYY-MM-DD
- **分类**：events/domain_knowledge/character_inspiration/scene_inspiration/other
- **适用阶段**：开书策划用/下批提纲用/以后备用/待定
- **使用状态**：未使用/已用于第X章/不采用（原因）
- **素材内容**：（原文或整理后的内容）
- **备注**：你的分析——适合怎么用、融入建议、风险点
```

### 使用规则

1. **素材优先级高于系统自动搜集的热点**：总编给的素材是最高优先级，提纲师写提纲时必须优先看materials/里有没有能用的
2. **素材不强制立刻用**：如果总编没指定"这个马上用"，可以先存着，合适的时候自然融入
3. **素材不能直接照抄**：要改编融入故事，不能把新闻原文直接贴进小说
4. **素材使用后更新状态**：某素材被用在某章后，更新素材文件的"使用状态"字段
5. **总编指定"这个用在下一批"时**：告诉提纲师必须考虑这个素材，但也要判断自然不自然——如果硬塞会违和，要如实告诉总编并建议调整方式

### 如果还没开书（阶段0/1/2）

总编在开书前就给素材：
- 先保存到 `work/_materials/` 临时目录（因为还没有novel_slug）
- 等开书确定novel_slug后，移到 `work/<novel_slug>/materials/` 对应目录
- 开书策划阶段，opening_planner必须参考这些素材
...
### 模式C：指定题材型
- 总编直接指定写什么题材/故事方向
- 流程：总编指定方向→选题策划（围绕方向出2-3个差异化方案）→总编选一个→补充调研→开书→循环写作
...
[2. 开书准备]（根据创作模式不同走不同路径）
   模式A：近期事件扫描（15-30天）→ 选题策划 → 【选题独立审稿→修改重审循环】→ 定题补充调研
          （可选：年度社会矛盾调研作为参考）
   模式B：选题策划（不依赖近期事件，基于风格+创意方向+市场地图）→ 【选题独立审稿→修改重审循环】→ 定题补充调研
   模式C：总编指定方向 → 选题策划（围绕方向出2-3个差异化方案）→ 【选题独立审稿→修改重审循环】→ 总编选一个 → 补充调研
   → 审阅通过
   ↓
[3. 开书策划]
   → 人物卡+世界观+结构设计+前20章粗纲+初始化动态状态文件
   → 【开书策划独立审稿→修改重审循环】→ 审阅通过
   → 初始化Novel Bible（复制风格规则、创建meta.yaml、Git提交）
   ↓
[4. 试写（首批N章，N由总编定）]
   → 写提纲（读取素材库materials/）→ 【提纲独立审稿→修改重审循环】→ 审阅通过
   → 写正文+内部审稿循环+状态更新+状态校验 → 修改/重写 → 审阅通过
   ↓
[5. 循环写作（每批N章）]
   → [模式A] 搜集近7日热点 → 审阅
   → 写下一批提纲（读取素材库materials/中未使用的素材）→ 【提纲独立审稿→修改重审循环】→ 审阅通过
   → 写正文+内部审稿循环+状态更新+状态校验 → 修改/重写 → 审阅通过
   → 处理总编反馈/素材 → 更新Bible/素材库
   → 总编判断是否完结 → 未完继续循环
   ↓
[6. 完结]
   → 完结审稿（伏笔回收/人物弧线/时间线/烂尾检查）→ 修改/补写 → 审阅通过
```
...
9. **选题审稿（第一轮）**：委派子Agent读 prompts/review/topic_reviewer.md，对选题方案做独立审稿
   - 审稿重点：AI味组合拳检查、读者吸引力≥7、真实性、差异化、长篇潜力
   - **必须检查输出文件末尾有没有自检结果**，没有的直接打回补自检
   - 根据审稿结果处理：
     a. 所有方案都不通过→让topic_planner重新出方案（必须在种子选择/主角位置上有根本变化，不能换汤不换药），重出后重审
     b. 部分方案有🔴问题→让topic_planner修改问题方案，修改后**只重审修改过的方案**
     c. 全部通过→进入下一步
   - 审稿→修改→重审循环最多2轮，2轮后仍有不通过的方案直接废弃
10. 呈现给总编：通过审稿的方案核心摘要、审稿报告、推荐排序
11. **【确认门】** 等总编选择或PASS：
    - 选了一个→进入补充调研
    - PASS/不满意→问清楚哪里不满意，让子Agent重新出方案（必须在种子选择/主角位置上有根本变化，不能换汤不换药）
12. 定题后补充调研：针对选中方案的待调研点深入搜索，保存到 `work/_research/supplementary_{novel_slug}.md`
13. **【确认门】** 等总编确认补充调研充分
14. Git提交。
...
3. **选题审稿（第一轮）**：委派子Agent读 prompts/review/topic_reviewer.md，对选题方案做独立审稿
   - 审稿重点：AI味组合拳检查、读者吸引力、真实性、差异化、是否偏离总编方向
   - **必须检查输出文件末尾有没有自检结果**，没有的直接打回补自检
   - 审稿→修改→重审循环最多2轮（同模式A/B规则）
4. 呈现给总编：通过审稿的方案核心摘要、审稿报告、推荐排序
5. **【确认门】** 等总编选择或PASS：
   - 选了一个→进入补充调研
   - PASS/不满意→问清楚哪里不满意，让子Agent重新出方案
6. 定题后补充调研：针对选中方案的待调研点深入搜索，保存到 `work/_research/supplementary_{novel_slug}.md`
7. **【确认门】** 等总编确认补充调研充分
8. Git提交。
...
2. 子Agent基于选题/指定方向+调研+风格规则layer0-5+总编提供的素材（materials/），制作开书策划：
   - **主角详细人物卡**：遵循layer3的人物塑造规则。包含姓名、外形、性格（优缺点并存）、说话方式、行为习惯、核心软肋、过往经历、底线、OOC红线（至少10条）
   - **核心配角卡**：数量参考layer3，每人同主角深度
   - **世界/场景设定**（world.md）：城市/地域、行当规则、主要活动场景、时代背景细节
   - **故事结构设计**：遵循layer4的结构规则。包含整体框架、主线/暗线设计、长线伏笔、各卷/各阶段核心冲突
   - **热点传导网络**（social_network.md，模式A/B要求，模式C可选）
   - **前20章粗纲**：每章含元数据（核心事件、章节定位、情感基调、伏笔操作、认知颠覆等级、章末钩子、出场人物）
   - **初始化动态状态文件**：character_state.md、plot_arcs.md、timeline.md、recent_summary.md
3. 保存到 work/<novel_slug>/bible/ 对应文件
4. **检查自检结果**：检查opening_plan.md末尾有没有opening_planner的自检清单（逐条打勾），没有的直接打回补自检
5. **开书策划审稿（第一轮）**：委派子Agent读 prompts/review/opening_plan_reviewer.md，对opening_plan.md做全面审稿
   - 审稿重点：AI组合拳套路检查（最优先级）、风格一致性、人物、入口事件与传导网络、暗线与伏笔、前20章节奏、反AI设计感、世界/行当真实性、生长空间
   - 审稿结果处理：
     a. 🔴不通过需重做→让opening_planner重新设计（思路问题，不是微调），重做后重审
     b. 🟡修改后再审→让opening_planner修改问题部分，修改后**重审整个策划案**（防止修改引入新问题）
     c. ✅通过→进入下一步
   - 审稿→修改→重审循环最多2轮，2轮后仍有🔴问题→向总编汇报，等指示
6. 呈现给总编：
   - 人设摘要、人物关系、故事框架、前20章粗纲
   - 开书审稿报告（所有🟡以上问题都要列出）
   - 总编提供的素材中哪些被采纳、哪些没用、为什么没用
7. **【确认门】** 等总编确认。有修改意见就改，改完后如果涉及核心情节/人物变化，要重跑opening_plan_reviewer审稿。
8. 确认通过后（审稿+总编双通过）：
   ...
   - 创建 materials/ 目录结构（events/domain_knowledge/character_inspiration/scene_inspiration/other/）
   - 把总编之前提供的临时素材（work/_materials/如果有）移到对应目录
9. Git提交。
...
4. 提纲最后附：本批情绪曲线、人物状态变化、时间推进、**八问自检结果（逐条打勾）**
5. 保存到 work/<novel_slug>/batch_plans/batch001_outline.md
6. **检查自检结果**：检查提纲末尾有没有outline_writer的八问自检清单（逐条打勾），没有的直接打回补自检
7. **提纲独立审稿**：委派子Agent读 prompts/review/outline_reviewer.md，对提纲做全面审稿
   - 审稿重点：AI组合拳套路检查（最优先级）、状态延续、人物OOC、伏笔操作、节奏、行当真实性、素材使用
   - 审稿结果处理：
     a. 🔴不通过需重做→让outline_writer重新设计，重做后重审
     b. 🟡修改后再审→让outline_writer修改问题部分，修改后重审
     c. ✅通过→进入下一步
   - 审稿→修改→重审循环最多2轮
8. **【确认门】** 呈现提纲+审稿报告给总编，等确认。总编有修改意见的，改完后涉及情节/人物变化要重跑outline_reviewer审稿。
...
8. 每章审稿通过后，**立即委派状态更新子Agent**：
   ...
   - 状态更新完成后，**委派逻辑审稿子Agent做状态校验**：对照本章正文和上一版状态，检查state_updater的更新有没有遗漏/错误（人物伤没记、钱数不对、伏笔漏了、时间错了等）
   - 校验不通过→让state_updater修正，修正后再校验
   - 校验通过后检查：有没有⚠️超期未回收的伏笔，记录下来给下一批提纲参考
9. 全部章节审稿通过+状态更新校验通过后：
   - 所有审稿记录保存到 work/<novel_slug>/reviews/batchXXX/ 目录（每章4个审稿报告+状态校验报告）
...
2. **写提纲**
   ...
   - 提纲写完后必须经过outline_reviewer独立审稿（同4.1的审稿-修改-重审循环）
   - 如果outline.md中已规划的章节点已经写完（需要扩展总大纲），outline_writer扩展的大纲部分也要经outline_reviewer审稿
   - **【确认门】** 等总编确认提纲（附审稿报告）

3. **写正文**
   - 同4.2，逐章写+审稿+修改+状态更新+状态校验
   - **【确认门】** 呈现正文等总编确认
...
总编说完结后：
1. **完结审稿**：委派子Agent做全面完结检查（读所有审稿Agent的规则综合判断）：
   - plot_arcs.md中所有伏笔是否回收？有没有🔴已埋未动或⚠️超期未回收的？
   - 主要人物弧线是否完整？人物结局是否符合人物性格？
   - 时间线是否连贯？有没有前后矛盾？
   - 暗线是否收束？有没有烂尾？
   - 未解决冲突是否都有交代（即使是开放式结局也要有道理）？
2. 完结审稿结果给总编，总编决定是否需要补写/修改
3. 需要补写/修改的，按正常写作流程走（提纲→审稿→正文→审稿→状态更新）
4. 生成全书目录和总字数统计
5. 最终Git提交，标记完结
6. 汇报：总字数、总章数、完结状态、伏笔回收情况、人物结局汇总
...
| **审稿-选题方案** | **prompts/review/topic_reviewer.md** |
| **审稿-开书策划** | **prompts/review/opening_plan_reviewer.md** |
| **审稿-提纲** | **prompts/review/outline_reviewer.md** |
| 审稿-文风 | prompts/review/style_reviewer.md |
| 审稿-逻辑（含状态校验） | prompts/review/logic_reviewer.md |
...
**最后更新：2026-07-01**
**v4.4变更（重大系统漏洞修复）：**
1. **补全提纲独立审稿环节**（修复用户指出的核心漏洞）：每批提纲写完后必须经outline_reviewer独立审稿，不再靠outline_writer自检
2. **补全所有阶段的审稿→修改→重审循环**：选题、开书策划、提纲三个阶段都明确了最多2轮的审稿-修改-重审机制
3. **强化AI组合拳套路检查**：把"醉后自语+神秘老人+眼神变了+说半句不说话"作为头号反面教材，所有策划/审稿Agent都重点打击多套路叠加的AI设计
4. **强制自检结果输出**：所有策划Agent（topic_planner/opening_planner/outline_writer）必须把自检清单逐条打勾附在输出末尾，没有自检直接打回
5. **增加状态更新校验**：state_updater更新完四个动态文件后，由logic_reviewer做独立校验，防止状态错误累积
6. **补全完结审稿**：完结阶段增加专门的完结审稿（伏笔回收/人物弧线/时间线/烂尾检查）
7. **20章后总纲扩展需审稿**：outline_writer扩展outline.md总大纲后，需经outline_reviewer审稿
8. **Prompt索引补全**：topic_reviewer、opening_plan_reviewer、outline_reviewer加入索引表
9. **素材库路径明确**：writer.md明确列出materials/路径，corrections.json要求必须读
10. **开书策划审稿通过后再初始化Bible**：防止未通过审稿就复制风格文件造成垃圾
11. **总编提修改意见后重审**：总编修改过的策划/提纲，如果涉及核心情节/人物变化，必须重跑审稿

**v4.2变更：补全模式C流程、world.md设定、outline.md创建、feedback目录等**
