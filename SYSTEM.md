...
**步骤**：
1. 委派子Agent读 prompts/research/hotspot_researcher.md，使用"年度调研模式"
2. 告诉子Agent：author_id是什么
3. 子Agent只读取layer0/1/2/5（不读layer3/4），广谱搜索过去12个月的社会趋势和代表性事件
4. 年度调研目标：10-15个代表性事件/趋势 + 3-5个核心社会矛盾群 + 5-8个社会情绪关键词 + 3-5个最佳小说切入面
5. 输出保存到 `work/_research/annual_{author_id}.md`（一份综合报告，不是零散的YAML文件）
6. 呈现给总编：核心矛盾群摘要、社会情绪、推荐切入点
7. **【确认门】** 等总编确认。方向偏了就调整，覆盖不够就补搜。
8. 确认通过后Git提交。
...
2. 告诉子Agent：author_id是什么
3. 子Agent读取layer0-5（layer3/4只读【通用规则】部分），模式A还需读取年度调研报告
4. 子Agent的任务：
   - 基于风格特征+（模式A：社会矛盾群；模式B：创意方向），头脑风暴至少6个候选职业
   - 对候选职业做WebSearch真实调研（收入、日常、行规、麻烦事、接触人群）
   - 评估每个职业的"事件敏感度"（≥7分才入选）和流动性
   - 设计热点传导机制（不是"听说新闻"，而是事件通过职业网络落到主角头上，主角面临两难）
   - 筛选出**至少4个差异化方案**（不同职业、不同城市、不同阶层、不同矛盾类型）
   - 每个方案包含：书名暂定、主角、职业真实调研（附来源）、城市、核心矛盾、核心配角、热点传导机制（≥5条）、暗线方向、开篇第一场景、事件敏感度评分、优劣势、待调研点、风格匹配度
   - 给出推荐排序和理由
5. 结果保存到 `work/_research/topic_proposals_{author_id}.md`
6. 呈现给总编：4个方案的核心摘要、推荐排序
7. **【确认门】** 等总编选择或PASS：
   - 选了一个→进入补充调研
   - PASS/不满意→问清楚哪里不满意，让子Agent重新出方案（必须在职业/城市/矛盾类型上有根本变化，不能换汤不换药）
8. 定题后补充调研：针对选中方案的待调研点深入搜索，保存到 `work/_research/supplementary_{novel_slug}.md`
9. **【确认门】** 等总编确认补充调研充分
10. Git提交。
